...
1. Define metadata information using the [metalite](https://merck.github.io/metalite/) R package. For detailed instructions and an example metadata object, please consult the
...
### Step 2: Prepare outdata using `prepare_*()` functions
```{r, eval = FALSE}
...
### Step 3: Format outdata using `format_*()` functions
```{r, eval=FALSE}
x <- x |> format_ae_summary() 
```

### Step 4: Create TLFs using `tlf_*()` functions.
```{r, eval=FALSE}
x |> 
  tlf_ae_summary(
