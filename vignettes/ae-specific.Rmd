...
[Chpater 8 of the R for Clinical Study Reports and Submission](https://r4csr.org/specific-ae.html) book. 
...
AE specification analysis using pre-specifed keywords in `meta`.
...
For example, we can use `extend_ae_specific_inference()` to add 95% confidence interval based on Miettinen and Nurminen (M&N) method. 
...
We can use `extend_ae_specific_events()` to add average duration of AE. 
Adding average number of AE per subject. It requires to use
...
The `tlf_ae_specific()` function also provides some commonly used argument to customize the table.
